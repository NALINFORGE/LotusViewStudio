"""Per-user Lotus View Studio frontend preferences."""
...
class LotusVisualPreferences:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self.legacy_store: Store[dict[str, Any]] = Store(
            hass, STORAGE_VERSION, LEGACY_STORAGE_KEY, private=True
        )
    ...
    async def async_load(self) -> None:
        ...
        if isinstance(legacy, dict) and isinstance(legacy.get("users"), dict):
            self.data = legacy
    ...
    async def async_set_language(self, user_id: str, language: str) -> dict[str, Any]:
        if language not in SUPPORTED_LANGUAGE_MODES:
            raise ValueError("Unsupported Lotus View Studio language")
...
def register_preference_websocket_commands(
    ...
    """Register per-user Lotus View Studio preference commands."""

    @websocket_api.websocket_command(
        {vol.Required("type"): "lotus_view_studio/preferences/get"}
    )
    ...
    @websocket_api.websocket_command(
        {
            vol.Required("type"): "lotus_view_studio/preferences/set",
    ...
    @websocket_api.async_response
    async def ws_get_legacy(hass, connection, msg):
        connection.send_result(msg["id"], manager.get_user(connection.user.id))
    ...
    @websocket_api.async_response
    async def ws_set_legacy(hass, connection, msg):
        try:
            result = await manager.async_set_language(
                connection.user.id, str(msg["language"])
            )
    ...
    websocket_api.async_register_command(hass, ws_get_legacy)
    websocket_api.async_register_command(hass, ws_set_legacy)
