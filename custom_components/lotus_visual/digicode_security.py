...
def register_websocket_commands(hass: HomeAssistant, manager: DigicodeSecurityManager) -> None:
    """Register Lotus Digicode WebSocket commands."""

    @websocket_api.websocket_command(
        {
            vol.Required("type"): "lotus_visual/digicode/status",
    ...
    @websocket_api.websocket_command(
        {vol.Required("type"): "lotus_visual/digicode/public_key"}
    )
    @websocket_api.async_response
    ...
    @websocket_api.websocket_command(
        {
            vol.Required("type"): "lotus_visual/digicode/save",
            vol.Required("pin_id"): cv.string,
    ...
    @websocket_api.websocket_command(
        {
            vol.Required("type"): "lotus_visual/digicode/validate",
            vol.Required("pin_id"): cv.string,
            vol.Required("mode"): vol.In(SERVER_MODES),
            vol.Optional("pin"): cv.string,
            vol.Optional("ciphertext"): cv.string,
        ...
        if result.get("valid"):
            if preview:
                ...
                try:
                    ...
                    _LOGGER.error("Validated Digicode action failed for %s: %s", msg["pin_id"], err)
                    result["action_executed"] = False
                    result["client_action"] = False
