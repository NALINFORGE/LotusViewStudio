...
const vscInternalToNative = (input, hass) => {
  ...
  for (const area of areas) {
    ...
    // while remaining completely harmless if Lotus Visual Stack is not installed.
...
class VisualStackCardEditor extends HTMLElement {
  ...
  _frameInspector(parent) {
    ...
    heading.innerHTML = `<div><h3>${lotusT("Contour")}</h3><p>${lotusT("Valeurs relatives et responsives. Elles définissent le ratio de conception et pourront servir de référence dans Lotus Visual.")}</p></div>`;
...
class LotusDynamicImageElement extends HTMLElement {
  ...
  _rawValue() {
    ...
    // still provide current_value. Lotus Visual itself no longer writes it.
