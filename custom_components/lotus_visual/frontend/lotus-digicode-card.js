...
async function encryptedPinPayload(hass, pin) {
  ...
  const response = await hass.callWS({ type: "lotus_visual/digicode/public_key" });
...
class LotusDigicodeCard extends HTMLElement {
  ...
  _handleVisibilityEntries(entries) {
    ...
    // conditional card inside Lotus Visual. Every genuine hidden -> visible
  ...
  async _loadServerSecurityStatus() {
    ...
    try {
      const status = await this._hass.callWS({ type: "lotus_visual/digicode/status", pin_id: pinId });
  ...
  async _validateServerPin() {
    ...
    try {
      const message = {
        type: "lotus_visual/digicode/validate",
        pin_id: pinId,
        mode,
...
class LotusDigicodeCardEditor extends HTMLElement {
  ...
  async _saveServerPin(button) {
    ...
    try {
      const message = {
        type: "lotus_visual/digicode/save",
        pin_id: pinId,
        mode,
  ...
  _renderSecuritySource(parent) {
    ...
    if (mode === DIGICODE_SECURITY_MODES.SERVER_PLAIN) {
      this._securityHelp(parent, "Comment enregistrer le code dans Home Assistant", [
        ...
        "Le PIN est alors conservé par l’intégration Lotus Visual côté serveur et n’apparaît ni dans le YAML de la carte ni dans les états Home Assistant.",
      ], "warning");
      ...
      this._securityHelp(parent, "Comment enregistrer le code dans Home Assistant", [
        ...
        "Saisissez le PIN dans le champ ci-dessous.",
        "Cliquez sur « Enregistrer le PIN et l’action sur le serveur ».",
        "Le navigateur chiffre immédiatement le PIN avec la clé publique éphémère de Lotus Visual avant l’envoi.",
  ...
  _render() {
    ...
    const source = this._section("Code et sécurité", "Choisissez où le PIN est conservé et comment il est validé. Les niveaux 2 et 3 effectuent la validation côté serveur Lotus Visual.");
    this._renderSecuritySource(source);
    this._renderSaveGuard(source);
    pane.appendChild(source);

    const format = this._section("Format responsive", "Le rapport largeur/hauteur sert de référence à Lotus Visual. Vous pouvez aussi tirer directement sur les quatre côtés de l’aperçu.");
    ...
    const interaction = this._section(
      "Interaction",
      "En mode blocage, lorsqu’un Digicode est visible dans une vue Lotus Visual, les cartes et commandes situées derrière deviennent inaccessibles jusqu’à la disparition du Digicode. Le Digicode reste le seul élément interactif."
    );
    this._boolean(
      interaction,
