...
class LotusVisualManager extends HTMLElement {
  ...
  _renderShell() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .page { max-width:1100px; margin:0 auto; padding:18px; box-sizing:border-box; }
        .topbar { position:sticky; top:0; z-index:20; display:flex; align-items:center; gap:8px; margin:-18px -18px 18px; padding:10px 18px; background:color-mix(in srgb, var(--lotus-bg) 96%, transparent); border-bottom:1px solid var(--lotus-border); backdrop-filter:blur(10px); }
        .brand { flex:1; min-width:0; display:flex; align-items:center; gap:10px; }
        .brand-logo { width:34px; height:34px; flex:0 0 34px; object-fit:contain; display:block; }
        .brand-text { min-width:0; }
        .brand-title { font-size:18px; font-weight:650; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .brand-subtitle { color:var(--lotus-muted); font-size:12px; }
        ...
        @media (max-width:650px) {
          .page { padding:10px; }
          .topbar { margin:-10px -10px 12px; padding:8px 10px; }
          .brand-subtitle, .dashboard-path { display:none; }
        ...
        <header class="topbar">
          <div class="brand">
            ...
            <div class="brand-text">
              <div class="brand-title">Lotus Visual</div>
              <div class="brand-subtitle">${lotusT("Éditeur visuel de dashboards")} · v${LOTUS_VISUAL_VERSION}</div>
            </div>
          </div>
    ...
    actions.append(
      ...
      makeIconButton({ icon: "mdi:view-dashboard-plus", title: "Créer un dashboard Lotus", className: "primary", onClick: () => this._showCreateDashboard() }),
    );
  }
  ...
  _renderContent() {
    ...
    notice.textContent = lotusT("Créez et convertissez vos vues ici. Pour le YAML brut, Lotus utilise exclusivement l’éditeur natif Home Assistant depuis le dashboard concerné.");
  ...
  _renderView(dashboard, config, view, index) {
    ...
    meta.textContent = `${view?.path || ""}${isLotus ? " · Lotus Visual" : ""}`;
    name.append(title, meta);
    const actions = document.createElement("div");
    actions.className = "actions";
    actions.append(
    ...
    if (!isLotus) {
      actions.appendChild(makeIconButton({
        icon: "lotus:lotus",
        title: "Convertir cette vue en vue Lotus Visual",
  ...
  _showLanguageDialog() {
    ...
    help.textContent = lotusT("En mode automatique, Lotus Visual utilise la langue de l’utilisateur connecté à Home Assistant. Une langue non disponible utilise l’anglais.");
  ...
  _showCreateDashboard() {
    this._showForm({
      ...
      fields: [
        { key: "title", label: lotusT("Nom"), value: "Lotus Visual" },
  ...
  async _createDashboard(values) {
    const title = values.title || "Lotus Visual";
